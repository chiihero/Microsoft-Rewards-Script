...
export default class BrowserUtils {
    ...
    async tryDismissAllMessages(page: Page): Promise<void> {
        try {
            ...
            if (overlay) {
                ...
                if (rejected) {
                    this.bot.logger.debug(this.bot.isMobile, 'DISMISS-ALL-MESSAGES', '已关闭：Bing 覆盖层拒绝')
                } else {
    ...
    async getLatestTab(page: Page): Promise<Page> {
        try {
            ...
            if (!newTab) {
                throw this.bot.logger.error(this.bot.isMobile, 'GET-NEW-TAB', '未找到任何标签页！')
            }
    ...
    async closeTabs(page: Page, config = { minTabs: 1, maxTabs: 1 }): Promise<Page> {
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'SEARCH-CLOSE-TABS',
                `发现 ${tabs.length} 个已打开的标签页（最小：${config.minTabs}，最大：${config.maxTabs}）`
            )
            ...
            if (tabs.length > config.maxTabs) {
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'SEARCH-CLOSE-TABS',
                    `已关闭 ${closedCount}/${tabsToClose.length} 个多余标签页，达到最大值 ${config.maxTabs}`
                )
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'SEARCH-CLOSE-TABS',
                    `正在打开 ${tabsNeeded} 个标签页，达到最小值 ${config.minTabs}`
                )
    ...
    async ghostClick(page: Page, selector: string, options?: ClickOptions): Promise<boolean> {
        try {
            ...
            this.bot.logger.warn(
                this.bot.isMobile,
                'GHOST-CLICK',
                `${selector} 失败：${error instanceof Error ? error.message : String(error)}`
            )
            return false
        }
    }
